                    ...
                    <div className="fixed inset-0 sm:top-0 sm:left-0 sm:right-auto sm:bottom-0 sm:w-80 bg-background text-popover-foreground border-r z-50 flex flex-col shadow-xl">
                      ...
                      <div className="flex-1 overflow-y-auto py-1">
                        {availableMcps.map((mcp) => {
                          ...
                          const isSkill = mcp.source === 'skill'
                          // Truncate description to ~60 chars for a functional summary
                          const shortDesc = mcp.description.length > 60
                            ? mcp.description.slice(0, 57) + '...'
                            : mcp.description
                          return (
                            <div
                              key={mcp.id}
                            ...
                            >
                              ...
                              <div
                                className={`flex-1 min-w-0 ${isSkill ? 'cursor-pointer group/skill rounded-md -mx-1 px-1 py-0.5 hover:bg-primary/5 transition-colors' : ''}`}
                                onClick={isSkill ? () => {
                                  setText(`/${mcp.name} `)
                              ...
                              >
                                <div className="flex items-center gap-1.5">
                                  <p className={`text-sm font-medium truncate ${isSkill ? 'group-hover/skill:text-primary transition-colors' : ''}`}>{mcp.name}</p>
                                ...
                                <div className="flex items-center gap-1">
                                  <p className="text-[10px] text-muted-foreground truncate">{shortDesc}</p>
                                  {isSkill && (
                                    <span className="shrink-0 text-[9px] text-primary/50 group-hover/skill:text-primary transition-colors">
                                      {/* Small arrow icon indicating clickable */}
                              ...
                              <div className="shrink-0 flex items-center gap-1.5">
                                ...
                                <button
                                  onClick={(e) => { e.stopPropagation(); onToggleMcp?.(mcp.id) }}
