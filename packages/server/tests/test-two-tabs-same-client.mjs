// Test: Two separate WebSocket connections with the exact same clientId (simulating two tabs)
...
function createTab(name, projectId, projectCwd, sharedClientId) {
  return new Promise((resolve) => {
    const ws = new WebSocket(`ws://localhost:4200/ws?token=${TOKEN}&clientId=${sharedClientId}`)
    const tab = { ws, name, projectId, ready: false, queryState: { started: false, firstDelta: false, ended: false, messages: 0 } }

    ws.on('open', () => {
      console.log(`[Tab ${name}] Connected with clientId ${sharedClientId}`)
...
async function main() {
  console.log('=== Creating two browser tabs with IDENTICAL clientId ===\n')
  const sharedClientId = `client-shared-${Date.now()}`
  const tabA = await createTab('A', PROJECT_A, '/Users/maxwin/workspace/test-projects/project-a', sharedClientId)
  const tabB = await createTab('B', PROJECT_B, '/Users/maxwin/workspace/test-projects/project-b', sharedClientId)
