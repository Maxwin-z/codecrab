...
SOUL 的 Identity Store 不再是一个自定义的文件管理类，而是一个**标准的 CodeClaws 项目**：

### 5.1 项目目录结构

```
~/.codeclaws/soul/                   ← 注册为内部项目 (id: "__soul__")
...
| Workspace | 复用 `~/.codeclaws/` 目录结构，新增 `workspace/` | 文件约定 |
...
**原因：** 把 `~/.codeclaws/soul/` 注册为项目后，Agent 可以直接用原生工具（Read/Edit/Write/Bash）操作其中的文件。无需维护 IdentityStore 类的 loadSoul/saveSoul 等方法——Agent 就是 store。同时天然获得 CLAUDE.md 行为约束、session 管理、模型配置等基础设施。
